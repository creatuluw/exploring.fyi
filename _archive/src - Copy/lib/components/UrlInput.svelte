<script lang="ts">
	...
	const handleSubmit = async () => {
		...
		console.log(`🚀 [UrlInput] Optimistic navigation for: "${trimmedUrl}"`);
		
		// Navigate immediately - this is the optimistic update
		await goto(`/explore?url=${encodeURIComponent(trimmedUrl)}`);
		
		// Background: Check for existing URL for future optimization
		// This runs after navigation so it doesn't block the UI
		try {
			console.log(`🔍 [UrlInput] Background check for existing URL: "${trimmedUrl}"`);
			const existingTopic = await TopicsService.findExistingTopicByUrl(trimmedUrl);
			
			if (existingTopic) {
				console.log(`✅ [UrlInput] Found existing URL topic in background: ${existingTopic.id}`);
				// Could potentially redirect to resume, but for now let the explore page handle it
			}
		} catch (error) {
			console.warn('Background URL check failed:', error);
	...
	<div class="space-y-4">
		...
		>
			{#if isLoading}
				...
				<span>{$t('url_input.loading')}</span>
			{:else}
				<ExternalLink class="h-5 w-5 mr-2" />
				<span>{$t('url_input.button')}</span>
			{/if}
		</button>
	</div>
