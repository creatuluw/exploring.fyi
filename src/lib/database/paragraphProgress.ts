...
export class ParagraphProgressService {
  ...
  static async markAsRead(
    ...
    try {
      // Check if topicId is a valid UUID, if not skip progress tracking for now
      const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(topicId);
      if (!isUuid) {
        console.log(`ℹ️ [Paragraph Progress] Skipping progress tracking for non-UUID topic: ${topicId}`);
