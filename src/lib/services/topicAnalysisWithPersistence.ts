...
export async function analyzeTopicStreamingWithPersistence(
  ...
  if (!sessionState.id) {
    console.log(`⏳ [AI+DB] Session not ready yet, waiting for session before proceeding`);
    
    // Wait for session to be ready
    const { isSessionReady } = await import('$lib/stores/session.js');
    await new Promise<void>((resolve) => {
      const unsubscribe = isSessionReady.subscribe((ready: boolean) => {
        if (ready) {
          unsubscribe();
          resolve();
        }
    ...
    if (!sessionState.id) {
      throw new Error('Session initialization failed - cannot save topic data');
    }
    
    console.log(`✅ [AI+DB] Session ready, proceeding with persistence for topic: "${topic}"`);
  }
  ...
  try {
    ...
    const enhancedOnProgress: StreamingCallback = async (data: StreamingNodeData) => {
      ...
      if (data.nodes.length > 0 && savedTopicId) {
        try {
          if (!savedMindMapId) {
            // First check if a mind map already exists for this topic
            const existingMindMap = await MindMapsService.getMindMapByTopic(savedTopicId);
            
            if (existingMindMap) {
              // Use existing mind map
              savedMindMapId = existingMindMap.id;
              console.log(`💾 [DB] Found existing mind map: ${savedMindMapId}`);
              // Update the existing mind map
              await MindMapsService.updateMindMapNodes(savedMindMapId, data.nodes, data.edges);
              console.log(`💾 [DB] Updated existing mind map: ${savedMindMapId}`);
            } else {
...
export async function analyzeUrlStreamingWithPersistence(
  ...
  if (!sessionState.id) {
    ...
    sessionState = get(session);
    if (!sessionState.id) {
      throw new Error('Session initialization failed - cannot save URL data');
    }
    
    console.log(`✅ [AI+DB] Session ready, proceeding with persistence for URL: "${url}"`);
  }
        ...
        try {
          if (!savedMindMapId) {
            ...
            if (existingMindMap) {
              // Use existing mind map
              savedMindMapId = existingMindMap.id;
              console.log(`💾 [DB] Found existing URL mind map: ${savedMindMapId}`);
              // Update the existing mind map
              await MindMapsService.updateMindMapNodes(savedMindMapId, data.nodes, data.edges);
              console.log(`💾 [DB] Updated existing URL mind map: ${savedMindMapId}`);
            } else {
              // Create new mind map record
              const mindMap = await MindMapsService.saveMindMap(
                savedTopicId,
...
/**
 * Load existing mind map by slug
 */
export async function loadMindMapBySlug(slug: string): Promise<StreamingNodeData | null> {
  try {
    ...
    return {
      ...
      edges: mindMap.edges as any[],
      isComplete: true,
      currentStep: 'Loaded from database via slug'
    };
  } catch (error) {
    console.error('Failed to load mind map by slug:', error);
    return null;
  }
}

/**
 * Load topic by slug with session context
 */
export async function loadTopicBySlug(sessionId: string, slug: string): Promise<any | null> {
  try {
    const topic = await TopicsService.getTopicBySlug(sessionId, slug);
    if (!topic) return null;

    // Enhance with mind map info
    let mindMapDetails = null;
    if (topic.mind_map_data && topic.mind_map_data.mindMapId) {
      try {
        const mindMap = await MindMapsService.getMindMapById(topic.mind_map_data.mindMapId);
        if (mindMap) {
          mindMapDetails = {
            id: mindMap.id,
            slug: mindMap.slug,
        ...
        console.warn('Failed to load mind map details for topic:', error);
      }
    }
    ...
    console.error('Failed to load topic by slug:', error);
    return null;
  }
}
